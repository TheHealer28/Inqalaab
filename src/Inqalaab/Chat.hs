...
import Inqalaab.Chat.Library.Commands
import Inqalaab.Chat.Operators
import Inqalaab.Chat.Operators.Presets
...
import Inqalaab.Chat.Util (shuffle)
...
defaultChatConfig =
  ChatConfig
    { agentConfig =
          ...
          { operators =
              [ PresetOperator
                  { operator = Just operatorSimpleXChat,
                    smp = inqalaabSMPServers, -- 4 Inqalaab SMP servers
                    useSMP = 4,
                    xftp = fluxXFTPServers, -- 4 Inqalaab XFTP servers
                    useXFTP = 4
                  },
                PresetOperator
                  { operator = Just operatorFlux,
                    smp = fluxSMPServers, -- empty: all servers under Inqalaab operator
                    useSMP = 0,
                    xftp = [], -- empty: all XFTP under Inqalaab operator
                    useXFTP = 0
                  }
              ],
      ...
      presetDomains = [".suchkitalash.info", ".inqalaab.chat"],
      tbqSize = 1024,
