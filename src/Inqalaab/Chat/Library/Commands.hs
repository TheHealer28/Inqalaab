...
module Inqalaab.Chat.Library.Commands where
...
import Inqalaab.Chat.Library.Subscriber
...
import Inqalaab.Chat.Remote
import Inqalaab.Chat.Remote.Types
import Inqalaab.Chat.Library.Internal
import Inqalaab.Chat.Stats
import Inqalaab.Chat.Store
import Inqalaab.Chat.Store.AppSettings
import Inqalaab.Chat.Store.ContactRequest
import Inqalaab.Chat.Store.Connections
...
import Inqalaab.Chat.Store.NoteFolders
...
import Inqalaab.Chat.Util (liftIOEither, zipWith3')
import qualified Inqalaab.Chat.Util as U
...
processChatCommand vr nm = \case
  ...
  CreateActiveUser NewUser {profile, pastTimestamp} -> do
    ...
    where
      ...
      createPresetContactCards db user = do
        createContact db user inqalaabStatusContactProfile
        createContact db user inqalaabSupportContactProfile
  ...
  ExportArchive -> do
    ...
    let filePath = "inqalaab." <> formatTime defaultTimeLocale "%FT%H%M%SZ" ts <> ".zip"
  ...
  ShowVersion -> do
    -- inqalbimqCommitQ makes iOS builds crash m(
          ...
          withFastStore' (\db -> getConnReqContactXContactId db vr user cReqHash1 cReqHash2) >>= \case
            Right ct@Contact {activeConn} -> case groupLinkId of
              Nothing -> case activeConn of
                Just conn@Connection {connStatus = ConnPrepared, xContactId} -> joinPreparedConn' xContactId conn False
                _ -> pure $ CVRConnectedContact ct
              Just gLinkId ->
...
chatCommandP =
  choice
    [ "/mute " *> ((`SetShowMessages` MFNone) <$> chatNameP),
      ...
      ("/support" <|> A.string ("/sim" <> "plex")) *> (ConnectSimplex <$> incognitoP),
