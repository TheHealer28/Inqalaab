...
module Inqalaab.Chat.View where
...
import Inqalaab.Chat.Help
import Inqalaab.Chat.Library.Commands (maxImageSize)
import Inqalaab.Chat.Markdown
import Inqalaab.Chat.Messages hiding (NewChatItem (..))
import Inqalaab.Chat.Messages.CIContent
import Inqalaab.Chat.Operators
import Inqalaab.Chat.Protocol
import Inqalaab.Chat.Remote.AppVersion (AppVersion (..), pattern AppVersionRange)
import Inqalaab.Chat.Remote.Types
import Inqalaab.Chat.Store (AddressSettings (..), AutoAccept (..), GroupLink (..), StoreError (..), UserContactLink (..))
import Inqalaab.Chat.Styled
import Inqalaab.Chat.Types
import Inqalaab.Chat.Types.Preferences
import Inqalaab.Chat.Types.Shared
import Inqalaab.Chat.Types.UITheme
...
viewConnReqInvitation (CCLink cReq shortLink) =
  ...
  where
    cReqStr = strEncode $ inqalaabChatInvitation cReq

inqalaabChatInvitation :: ConnReqInvitation -> ConnReqInvitation
inqalaabChatInvitation (CRInvitationUri crData e2e) = CRInvitationUri crData {crScheme = simplexChat} e2e
...
connReqContact_ intro (CCLink cReq shortLink) =
  ...
  where
    cReqStr = strEncode $ inqalaabChatContact cReq

inqalaabChatContact :: ConnReqContact -> ConnReqContact
inqalaabChatContact (CRContactUri crData) = CRContactUri crData {crScheme = simplexChat}

inqalaabChatContact' :: ConnLinkContact -> ConnLinkContact
inqalaabChatContact' = \case
...
viewContactInfo ct@Contact {contactId, profile = LocalProfile {localAlias, contactLink}, activeConn, uiThemes, customData} stats incognitoProfile =
  ["contact ID: " <> sShow contactId]
    ...
    <> maybe [] (\l -> ["contact address: " <> (plain . strEncode) (inqalaabChatContact' l)]) contactLink
    <> maybe
...
viewCallInvitation ct@Contact {contactId} callType@CallType {media} sharedKey =
  [ ttyContact' ct <> " wants to connect with you via WebRTC " <> callMediaStr callType <> " call " <> encryptedCallText callType,
    ...
    "",
    "https://inqalaab.chat/call#" <> plain queryString
  ]
  where
...
viewVersionInfo logLevel CoreVersionInfo {version, inqalbimqVersion, inqalbimqCommit} =
  map plain $
    if logLevel <= CLLInfo
      then [versionString version, updateStr, "message core: " <> inqalbimqVersion <> parens inqalbimqCommit]
