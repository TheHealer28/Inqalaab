...
func ntfModeDescription(_ mode: NotificationsMode) -> LocalizedStringKey {
    switch mode {
    case .off: return "**Most private**: do not use push server. The app will check messages in background, when the system allows it, depending on how often you use the app."
    case .periodic: return "**More private**: check new messages every 20 minutes. Only device token is shared with the push server. It doesn't see how many contacts you have, or any message metadata."
    case .instant: return "**Recommended**: device token and end-to-end encrypted notifications are sent to the push server, but it does not see the message content, size or who it is from."
    }
}
